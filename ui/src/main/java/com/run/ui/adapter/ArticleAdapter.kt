...
import com.run.ui.R
import com.run.ui.R.id.selectView
...
import kotlinx.android.synthetic.main.item_article_text_layout.view.*
...
class ArticleAdapter : BaseMultiItemQuickAdapter<ArticleBean, BaseViewHolder>(null) {
    ...
    override fun convert(helper: BaseViewHolder, item: ArticleBean) {//设置头像
        ...
        when (helper.itemViewType) {
            ArticleBean.ARTICLE_TEXT -> {
                ...
                val selectView: ImageView = helper.getView(R.id.selectView)
                if (canSelect) {
                ...
                selectView.isSelected = item.article_id in selectList



                helper.getView<View>(R.id.rl_root).setOnClickListener {
                    if (canSelect) {
                        ULog.d("Xioaruan", item.article_id)
                        if (item.article_id in selectList) {
                            selectView.isSelected = false
                            selectList.remove(item.article_id)
                        } else {
                            if (selectList.size >= 10) {
                                Toast.makeText(mContext, "每次操作最多只能复制十篇文章", Toast.LENGTH_SHORT).show()
                                return@setOnClickListener
                            }
                            selectView.isSelected = true
                            selectList.add(item.article_id)
                        }
    ...
    var selectList: ArrayList<Int> = arrayListOf()
